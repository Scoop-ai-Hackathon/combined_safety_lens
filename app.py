...
class SafetyVideoProcessor(VideoProcessorBase):
    ...
    def recv(self, frame):
        ...
        if self.hazard_latched:
            ...
            cv2.putText(img, "SYSTEM STOPPED", (30, 60), cv2.FONT_HERSHEY_SIMPLEX, 1.2, (0,0,255), 3)
            self.frame = img
        ...
        if self.prev_gray is not None:
            flow = cv2.calcOpticalFlowFarneback(self.prev_gray, gray, None, 0.5, 2, 10, 2, 5, 1.1, 0)
        self.prev_gray = gray

        compression = motion = hand_detected = False

        if self.detector:
            rgb = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
            det = self.detector.detect(mp.Image(image_format=mp.ImageFormat.SRGB, data=rgb))
            if det.hand_landmarks:
                ...
                area = (x2-x1) * (y2-y1)
                if self.prev_area and (area - self.prev_area) / max(1, self.prev_area) < -0.05:
                    compression = True
                self.prev_area = area

                if flow is not None:
                    sx, sy = 320/w, 240/h
                    roi = flow[int(y1*sy):int(y2*sy), int(x1*sx):int(x2*sx)]
                    if roi.size > 0 and np.mean(np.abs(roi)) > 3.0:
                        motion = True

        if hand_detected and (compression or motion):
            self.hazard_count = min(self.hazard_count + 2, 15)
        else:
            self.hazard_count = max(0, self.hazard_count - 1)

        if hand_detected:
            cv2.putText(img, f"MONITORING - Hazard: {self.hazard_count}/5", (30,40), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0,255,255), 2)
        else:
            cv2.putText(img, "ZONE CLEAR", (30,40), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (100,255,100), 2)

        if self.hazard_count >= 5 and not self.hazard_latched:
...
def render_realtime_sentinel():
    ...
    if detector is None:
        st.warning("hand_landmarker.task not found.")
